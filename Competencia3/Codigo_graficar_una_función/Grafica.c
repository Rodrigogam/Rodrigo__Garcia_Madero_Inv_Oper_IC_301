...
void graficar(int r1, int r2,int m, int b, int t){
	...
	//iniciar Gnuplot, inicio sesion
    inicioS = gnuplot_init();
    
    gnuplot_setstyle(inicioS, "lines"); 
	//aspecto
    gnuplot_cmd(inicioS,"set xzeroaxis lt -1");
	//coloca el eje x
	gnuplot_cmd(inicioS,"set yzeroaxis lt -1");
	//coloca el eje y
    gnuplot_set_ylabel(inicioS, "Eje X");
	//nombre de las etiquetas en y
    gnuplot_set_xlabel(inicioS, "Eje Y");
	//nombre de las etiquetas en x
    gnuplot_cmd(inicioS,"set xrange [%d:%d]",r1, r2);
	//define el rango x
    gnuplot_cmd(inicioS,"set yrange [%d:%d]",r1,r2 );
	//define el rango y
    gnuplot_plot_slope(inicioS,m,b, "--Funcion de la forma y=mx+b--"); 
	//crea el grafico con la funcion
    
    sleep(t);
	
	gnuplot_close (inicioS);
	//cerrar la sesion para elliminar posibles archivos residuales
